...
1. **Read before you touch anything**: CLAUDE.md first — its invariants and
   ...
   what. Then read every file you intend to change.
...
3. **Off limits, no exceptions**: secrets.py, files.py, tools/__init__.py,
   approvals.py, permissions.py — the layers that gate you. write_file
   refuses them; do not route around it with shell commands. If the
   improvement needs one of them changed, explain exactly what and why, and
   let the owner do it.
4. Make the change. Match the codebase's style; keep the loop small; update
   CLAUDE.md in the same session if you added a capability or invariant.
